...
class InventoryWindow:
    ...
    def select_image(self):
        filepath = filedialog.askopenfilename(title="Chọn ảnh sản phẩm", filetypes=[("Image Files", "*.png;*.jpg;*.jpeg")])
        if filepath:
    ...
    def add_product(self):
        ...
        if not pid:
            messagebox.showerror("Lỗi", "Mã ID không được để trống")
            return
        if any(p['ID'] == pid for p in self.products):
            messagebox.showerror("Lỗi", "Mã ID đã tồn tại")
            return

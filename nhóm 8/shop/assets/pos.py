...
class POSWindow:
        ...
        for c in self.cart:
            if c['ID'] == pid:
                if c['Qty'] < stock:
                    ...
                    self.update_cart_ui()
                else: messagebox.showwarning("Cảnh báo", "Không đủ số lượng", parent=self.root)
                return
    ...
    def update_cart_ui(self):
        ...
        self.lbl_total.configure(text=f"Tổng tiền: {total}")

    def checkout(self):
        if not self.cart: return messagebox.showinfo("Thông báo", "Giỏ hàng trống", parent=self.root)
        for c in self.cart:
            for p in self.products:
                if p['ID'] == c['ID']:
        ...
        messagebox.showinfo("Thành công", "Thanh toán hoàn tất!", parent=self.root)